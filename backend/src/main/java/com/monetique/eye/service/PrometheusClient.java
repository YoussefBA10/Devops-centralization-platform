...
public class PrometheusClient {
    ...
    public Double getDiskUsagePercent(String envLabel) {
        String query = String.format(
            "max(1 - (" +
            "(node_filesystem_avail_bytes{mountpoint=\"/data\", environment=\"%s\"} or ignoring(mountpoint) node_filesystem_avail_bytes{mountpoint=\"/\", environment=\"%s\"}) / " +
            "(node_filesystem_size_bytes{mountpoint=\"/data\", environment=\"%s\"} or ignoring(mountpoint) node_filesystem_size_bytes{mountpoint=\"/\", environment=\"%s\"})" +
            ")) * 100", 
