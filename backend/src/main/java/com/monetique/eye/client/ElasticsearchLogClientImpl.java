    ...
    public java.util.Map<String, Object> fetchSreSignals(String envLabel, String appFilter, java.time.Instant from, java.time.Instant to) {
        ...
        try {
            SearchResponse<Void> response = esClient.search(s -> s
                ...
                .aggregations("unhandled_exception", a -> a.filter(f -> f.bool(b -> b.should(s1 -> s1.match(m -> m.field("message").query("Unhandled exception"))).should(s2 -> s2.match(m -> m.field("message").query("Fatal application error"))))))
                .aggregations("runtime_exception_count", a -> a.filter(f -> f.bool(b -> b.should(s1 -> s1.term(t -> t.field("exception_type.keyword").value("java.lang.RuntimeException"))).should(s2 -> s2.match(m -> m.field("message").query("RuntimeException"))))))
                .aggregations("illegal_state_count", a -> a.filter(f -> f.bool(b -> b.should(s1 -> s1.term(t -> t.field("exception_type.keyword").value("java.lang.IllegalStateException"))).should(s2 -> s2.match(m -> m.field("message").query("IllegalStateException"))))))
                .aggregations("startup_failure", a -> a.filter(f -> f.bool(b -> b.should(s1 -> s1.match(m -> m.field("message").query("Application context initialization failed"))).should(s2 -> s2.match(m -> m.field("message").query("Spring Boot startup"))))))
