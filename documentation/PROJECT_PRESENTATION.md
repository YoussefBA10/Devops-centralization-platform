...
| **Security Dashboard** | Global vulnerability posture, OWASP/SonarQube insights, Falco runtime threats | All |
| **Analyse Dashboard** | Historical incident trends, AI root cause analysis, log correlation | All |
...
| **Harbor** | Container Registry | Secure, private image hosting with vulnerability scanning |
...
| **Shell Scripts** | Operational automation | SSH config, standalone agent deployment, RCA signal simulation (`simulate_rca_signals.sh`) |
...
## 5.7 DevSecOps & Security

| Technology | Role | Why Chosen |
|:---|:---|:---|
| **HashiCorp Vault** | Secrets Management | Secure, dynamic, and centralized key/credential storage |
| **Falco** | Runtime Security | Deep kernel-level threat detection (eBPF) and security alerting |
| **OWASP Dependency-Check** | SCA | Software Composition Analysis during CI/CD to detect known CVEs |
| **SonarQube** | SAST & Code Quality | Static analysis for bugs, code smells, and security vulnerabilities |
...
├── 📂 controller/      # 32 REST controllers (API endpoints)
├── 📂 dto/             # 16 Data Transfer Objects
├── 📂 entity/          # 28 JPA entities + 6 enums
├── 📂 repository/      # 28 Spring Data JPA repositories
├── 📂 service/         # 36 business services
├── 📂 security/        # AOP permissions, Falco integration, Vulnerability scanning
...
|:---|:---|:---|
| **DeploymentService** | ~64KB | Full application lifecycle: deploy, undeploy, restart, canary, promote |
| **SecurityReportService** | ~53KB | OWASP/SonarQube vulnerability ingestion, aggregation, and scoring |
| **LogAnalyticsService** | ~41KB | Log aggregation, trend analysis, recurring pattern detection |
| **AiChatService** | ~38KB | Context-aware AI chat with conversation history and real-time metrics |
| **InfrastructureService** | ~28KB | Topology generation, service resource discovery, heatmaps, stability score |
| **RootCauseIntelligenceService** | ~19KB | Automated root cause analysis and correlation |
| **PrometheusClient** | ~17KB | PromQL query execution, metric aggregation, node/container metrics |
| **NetworkMetricsProxyService** | ~14KB | Network topology, latency, packet loss monitoring |
| **AlertGroupService** | ~10KB | Alert grouping, AI-powered root cause analysis |
| **FalcoEventService** | ~8KB | Runtime security event ingestion and threat alerting |
| **OperationalScheduler** | ~5KB | Automated routine health checks, log rotations, and scheduled config backups |
| **CicdTrackingService** | ~4KB | Deployment event recording and VictoriaMetrics integration |
| **PermissionService** | ~3KB | Fine-grained RBAC permission checks |
...
├── 📂 pages/ (22 pages)    # Full page components
...
| **Secrets Management** | HashiCorp Vault | Dynamic secrets, secure key storage, Spring Cloud Vault integration |
| **Container Registry** | Harbor | Secure image hosting, CVE scanning, role-based access |
...
| **Runtime Security** | Falco | Deep container threat detection at the kernel/eBPF level |
