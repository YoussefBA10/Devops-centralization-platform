...
On a personal note, I couldn't have done this without my family. They've supported me through every late night of debugging and every stressful deadline. Finally, a special thank you to my fiancée. Your patience and encouragement kept me going when the code wouldn't compile and the solutions felt out of reach. This accomplishment is as much yours as it is mine.
...
\begin{itemize}
  ...
  \item \textbf{Access Control Challenges}: Most monitoring platforms offer only large gained RBAC, making it difficult to enforce environment-level data isolation in multi-tenant scenarios.
...
Monetique is a leading enterprise software company specializing in digital payment solutions and infrastructure services for the financial sector. The company manages critical payment processing systems, transaction platforms, and enterprise middleware for financial institutions and retailers.
...
Additionally, provisioning new monitoring infrastructure was slow: adding a client node for monitoring required a DevOps engineer to manually configure SSH keys, Ansible inventories, Prometheus targets, and logging agents—a process that typically took 8-10 hours per node.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{"Clean C4Context Diagram.png"}
...
Traditional infrastructure monitoring suffers from what we call « observability fragmentation »: data lives in disparate, and operators must manually jump between tools. Consider a typical incident:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{"GLOBAL_ARCHITECTURE_DIAGRAM_2.png"}
...
\begin{description}
  \item[Structured Data Model] 25 entities organized by domain, balancing completeness with maintainability
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{"Diagram de gantt.png"}
...
The data model grew organically through the five phases, ending up at 25 JPA entities. We grouped them by functional domain to keep things organized:

\textbf{Core Infrastructure (6 entities):}
\begin{center}
\begin{tabular}{|l|l|p{4cm}|}
  \hline
  \textbf{Entity} & \textbf{Purpose} & \textbf{Key Fields} \\
  \hline
  \texttt{Environment} & Top-level data container & name, cluster, labels \\
  \hline
  \texttt{Application} & Deployed service in an environment & name, type, status \\
  \hline
  \texttt{Cluster} & Physical/logical infrastructure group & name, description, nodes \\
  \hline
  \texttt{ClusterAccess} & SSH credentials for cluster nodes & host, sshKey, username \\
  \hline
  \texttt{ManagedNode} & Individual monitored server/VM & hostname, ip, environment \\
  \hline
  \texttt{ServiceLink} & Dependency mapping between services & source, target, type \\
  \hline
\end{tabular}
\end{center}

\textbf{Security \& Access Control (3 entities):}
\begin{center}
\begin{tabular}{|l|l|p{4cm}|}
  \hline
  \textbf{Entity} & \textbf{Purpose} & \textbf{Key Fields} \\
  \hline
  \texttt{User} & Platform user with credentials & username, email, passwordHash \\
  \hline
  \texttt{UserPermission} & Role assignment per environment & userId, environmentId, role \\
  \hline
  \texttt{UserPermissionDetail} & Fine-grained permission flags & permissionId, resource, action \\
  \hline
\end{tabular}
\end{center}

\textbf{Incident \& Alert Management (7 entities):}
\begin{center}
\begin{tabular}{|l|l|p{4cm}|}
  \hline
  \textbf{Entity} & \textbf{Purpose} & \textbf{Key Fields} \\
  \hline
  \texttt{Incident} & Auto-detected system incident & severity, description, timestamp \\
  \hline
  \texttt{IncidentTimelineEntry} & Chronological incident events & incidentId, event, timestamp \\
  \hline
  \texttt{IncidentAttachment} & Files/logs attached to incidents & incidentId, filename, content \\
  \hline
  \texttt{Alert} & Raw alert from Alertmanager & name, status, labels \\
  \hline
  \texttt{AlertGroup} & Grouped alerts by rule & groupKey, alerts, receiver \\
  \hline
  \texttt{NetworkAlertRule} & Custom network monitoring rules & name, condition, threshold \\
  \hline
  \texttt{Ticket} & Operator-created incident tickets & title, status, priority \\
  \hline
\end{tabular}
\end{center}

\textbf{Deployment \& CI/CD (3 entities):}
\begin{center}
\begin{tabular}{|l|l|p{4cm}|}
  \hline
  \textbf{Entity} & \textbf{Purpose} & \textbf{Key Fields} \\
  ...
  \hline
  \texttt{DeploymentEvent} & CI/CD pipeline events & pipelineId, commit, timestamp \\
  \hline
  \texttt{GitHubToken} & OAuth tokens for Git integration & token, repository, expiry \\
  \hline
\end{tabular}
\end{center}

\textbf{AI \& Observability Intelligence (4 entities):}
\begin{center}
\begin{tabular}{|l|l|p{4cm}|}
  \hline
  \textbf{Entity} & \textbf{Purpose} & \textbf{Key Fields} \\
  ...
  \hline
  \texttt{LogAggregationWindow} & Stability score windows & environment, window, score \\
  \hline
...
\textbf{Platform Operations (2 entities):}
\begin{center}
\begin{tabular}{|l|l|p{4cm}|}
  \hline
  \textbf{Entity} & \textbf{Purpose} & \textbf{Key Fields} \\
  \hline
  \texttt{Notification} & In-app user notifications & userId, message, read \\
  \hline
  \texttt{ActivityLog} & Audit log of all user actions & userId, action, timestamp \\
  \hline
\end{tabular}
\end{center}

All entities are scoped to an \texttt{Environment} where applicable, enforcing multi-tenancy at the database level. Raw observability data (metrics, logs) lives in Prometheus and Elasticsearch respectively---the MySQL database handles orchestration and platform state only.
...
\begin{description}
  ...
  \item[USER] Scoped access to assigned environments provided by the admin; can view dashboards, edit assigned environments (e.g., production), view logs, and create tickets
\end{description}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{"Operational Intelligence Dashboard Component Layout Diagram.png"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{"Infrastructure Visibility \& Topology Flow.png"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{"Sequences_Environment_Management.png"}
...
When an operator clicks ``Deploy'' in the UI, they choose between two modes:

\begin{description}
  \item[Containerized Mode] The default. The backend runs an Ansible playbook (\texttt{deploy-tools.yml}) that installs Node Exporter, cAdvisor, and Filebeat as Docker containers on the target node. This requires Docker and Python 3 on the remote machine.
  \item[Standalone Mode] For servers without Docker. The backend runs a raw SSH script (\texttt{deploy-standalone.sh}) that deploys Node Exporter and Process Exporter as systemd user services. It requires \textbf{zero dependencies} on the remote---no Python, no Docker, no package manager. Just bash and systemd.
\end{description}

The \texttt{DeploymentService} checks a \texttt{containerized} boolean flag from the UI request and branches accordingly:

\begin{itemize}
  \item If \texttt{containerized = true}: execute Ansible playbook with \texttt{-e containerized=true}
  \item If \texttt{containerized = false}: execute the standalone bash script via SSH
\end{itemize}

The standalone script works in 9 steps: verify SSH connectivity, cache the Node Exporter binary locally, transfer it via SCP, extract it on the remote host, write a systemd user service file, enable and start the service, and verify it's running. If \texttt{systemctl --user} isn't available (some older systems don't support it), the script falls back to \texttt{nohup} as a last resort.

Each deployed node gets a \texttt{ManagedNode} entity in the database with a \texttt{containerized} flag, so the platform always knows how a node was provisioned.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{"Diagram 1 — Root Cause Signal pipeline.png"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{"Operational Intelligence Dashboard Stability Scoring Flow.png"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{"FULL_BACKEND_CLASS_DIAGRAM.png"}
  \caption{Full Backend Class Diagram: 25 Entities and Relationships}
...
\begin{figure}[H]
  \centering
  \includesvg[width=0.75\textwidth]{"RBAC \& Multi-Tenancy Diagram"}
...
\begin{figure}[H]
  \centering
  \includesvg[width=0.65\textwidth]{"Ticket Lifecycle State Diagram"}
...
\begin{figure}[H]
  \centering
  \includesvg[width=0.75\textwidth]{"Infrastructure Visibility \& Topology Entity Link"}
...
\textbf{Problem:} Early designs mixed raw observability data (metrics, log entries) with platform orchestration data. This would have meant duplicating what Prometheus and Elasticsearch already store, and made the schema bloated and hard to maintain.

\textbf{Solution:} We drew a clear boundary: MySQL handles platform state and orchestration (environments, deployments, incidents, users, AI summaries). Raw metrics stay in Prometheus; raw logs stay in Elasticsearch. The backend queries these external systems on demand rather than copying data into MySQL. This kept the 25-entity model focused and each entity meaningful.

\textbf{Lesson:} Knowing what not to store is just as important as the schema design itself. The separation between orchestration data and observability data kept the system clean.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{"monitoring_process_diagram.png"}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{"Application Management Relationship Diagram.png"}
...
When I started this internship, Monetique's infrastructure team had a clear but difficult problem. Their observability tools were disconnected, diagnosing incidents took too long, and adding monitoring to new servers was a huge chore. My goal was to build a single platform that tied everything together and made it smarter.

Looking back at the last six months, I'm proud of what we achieved with Monetique Eye. We didn't just build a dashboard. We built an automated pipeline that can provision a new monitoring agent in two minutes instead of four hours. We built a correlation engine that actually helps operators figure out why a server is failing, rather than just telling them that it failed.

This wasn't an easy project. Getting the environment-level access control right took several rewrites. Making the AI analysis feel fast and responsive required moving to an asynchronous architecture that I hadn't planned for. There were days when the Prometheus integrations just wouldn't work, and I had to dig deep into the documentation to figure out why.

But the result is a platform that the team can actually use. They don't have to jump between three different tools to understand an incident anymore. The AI integration, which started as an experiment, turned out to be one of the most useful features for quickly summarizing what went wrong.

If I were to continue working on this, I'd want to focus on predictive analytics—getting the system to warn us before an incident happens, rather than just diagnosing it faster. I'd also want to expand the deployment automation to handle Kubernetes clusters natively, not just standalone VMs.

This internship taught me what it actually takes to build enterprise software. Writing the code is only half the job. The other half is understanding the user's workflow, making the system secure, and ensuring it can handle real production data without falling over. I'm leaving this project as a much better engineer than when I started, and I'm excited to see where the team takes Monetique Eye next.
