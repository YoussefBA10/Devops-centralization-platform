...
const DashboardPage: React.FC = () => {
  ...
  const stats = [
    { name: 'Active Environments', value: environments.length, icon: Layers, trend: '+12%', trendUp: true, color: 'text-primary' },
    { name: 'Total Nodes', value: '14', icon: Server, trend: '+2', trendUp: true, color: 'text-emerald-500' },
    { name: 'Stability Index', value: '98%', icon: Activity, trend: '-0.5%', trendUp: false, color: 'text-amber-500' },
    { name: 'Open Tickets', value: '3', icon: FileText, trend: '-4', trendUp: true, color: 'text-primary' },
  ];

  const recentActivity = [
    { title: 'Environment Deployed', type: 'system', time: '2h ago', env: 'Production' },
    { title: 'New Ticket: Node-01 High Load', type: 'incident', time: '4h ago', env: 'Staging' },
    { title: 'Agent Health Resolved', type: 'system', time: 'Yesterday', env: 'Dev' },
  ];
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <Card className="bg-emerald-950/10 border-emerald-500/10">
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3 font-mono text-[11px] text-emerald-500/60 leading-relaxed">
                    <p>{'>'} STABLE-AGENT: node-01 sync complete [0.2ms]</p>
                    <p>{'>'} METRIC-DB: write success (id: 4859a)</p>
                    <p className="text-emerald-500">{'>'} SYS-OK: all services operational [prod-env]</p>
                  </div>
                </CardContent>
              </Card>
              <Card>
