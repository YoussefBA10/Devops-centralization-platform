...
export const QUERIES = {
  ...
  CPU_THROTTLING: (_appId: string, appName: string, nodeId: string, _node: string) => 
    `rate(container_cpu_usage_seconds_total{name=~".*${appName}.*", node_id="${nodeId}"}[5m]) * 100`,

  // B. CONTAINER RESTARTS (Changes in start time)
  CONTAINER_RESTARTS: (_appId: string, appName: string, nodeId: string, _node: string) => 
  ...
  OOM_EVENTS: (_appId: string, appName: string, nodeId: string, _node: string) => 
    `container_oom_events_total{name=~".*${appName}.*", node_id="${nodeId}"}`,

  // E. NETWORK PACKET DROPS (Host level)
  NETWORK_DROPS: (nodeId: string, _node: string) => ({
  ...
  LOAD_AVERAGE_RATIO: (nodeId: string, _node: string) => 
    `(node_load1{node_id="${nodeId}"} / count without(cpu, mode) (node_cpu_seconds_total{node_id="${nodeId}", mode="idle"}))`,

  // H. CONTAINER UPTIME
  CONTAINER_UPTIME: (_appId: string, appName: string, nodeId: string, _node: string) => 
    `time() - container_start_time_seconds{name=~".*${appName}.*", node_id="${nodeId}"}`,

  // I. CPU USAGE TREND (Stacked Area)
  CPU_USAGE_STACKED: (_appId: string, appName: string, nodeId: string, _node: string) => 
    `rate(container_cpu_usage_seconds_total{name=~".*${appName}.*", node_id="${nodeId}"}[5m]) * 100`,

  // J. NETWORK THROUGHPUT
  NETWORK_THROUGHPUT: (_appId: string, appName: string, nodeId: string, _node: string) => ({
  ...
  // K. NODE INFO
  NODE_INFO: (nodeId: string, _node: string) => 
    `node_uname_info{node_id="${nodeId}"}`,
  
  NODE_RESOURCES: (nodeId: string, _node: string) => ({
