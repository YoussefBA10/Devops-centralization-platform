...
export const QUERIES = {
  ...
  CPU_THROTTLING: (appId: string, appName: string, nodeId: string, node: string) =>
    `(sum(rate(container_cpu_cfs_throttled_periods_total{container_label_com_monetique_app_id="${appId}", node_id="${nodeId}"}[5m])) / sum(rate(container_cpu_cfs_periods_total{container_label_com_monetique_app_id="${appId}", node_id="${nodeId}"}[5m]) > 0) * 100) or (sum(rate(container_cpu_cfs_throttled_seconds_total{container_label_com_monetique_app_id="${appId}", node_id="${nodeId}"}[5m])) / sum(rate(container_cpu_usage_seconds_total{container_label_com_monetique_app_id="${appId}", node_id="${nodeId}"}[5m]) > 0) * 100) or (sum(rate(container_cpu_throttled_seconds_total{container_label_com_monetique_app_id="${appId}", node_id="${nodeId}"}[5m])) / sum(rate(container_cpu_usage_seconds_total{container_label_com_monetique_app_id="${appId}", node_id="${nodeId}"}[5m]) > 0) * 100) or (sum(rate(container_cpu_cfs_throttled_periods_total{name=~".*${appName}.*", node_id="${nodeId}"}[5m])) / sum(rate(container_cpu_cfs_periods_total{name=~".*${appName}.*", node_id="${nodeId}"}[5m]) > 0) * 100) or (sum(rate(container_cpu_cfs_throttled_seconds_total{name=~".*${appName}.*", node_id="${nodeId}"}[5m])) / sum(rate(container_cpu_usage_seconds_total{name=~".*${appName}.*", node_id="${nodeId}"}[5m]) > 0) * 100) or (sum(rate(container_cpu_throttled_seconds_total{name=~".*${appName}.*", node_id="${nodeId}"}[5m])) / sum(rate(container_cpu_usage_seconds_total{name=~".*${appName}.*", node_id="${nodeId}"}[5m]) > 0) * 100) or (sum(rate(container_cpu_cfs_throttled_periods_total{name=~".*${appName}.*", instance=~"${node}(:.*)?"}[5m])) / sum(rate(container_cpu_cfs_periods_total{name=~".*${appName}.*", instance=~"${node}(:.*)?"}[5m]) > 0) * 100) or (sum(rate(container_cpu_cfs_throttled_seconds_total{name=~".*${appName}.*", instance=~"${node}(:.*)?"}[5m])) / sum(rate(container_cpu_usage_seconds_total{name=~".*${appName}.*", instance=~"${node}(:.*)?"}[5m]) > 0) * 100) or (sum(rate(container_cpu_throttled_seconds_total{name=~".*${appName}.*", instance=~"${node}(:.*)?"}[5m])) / sum(rate(container_cpu_usage_seconds_total{name=~".*${appName}.*", instance=~"${node}(:.*)?"}[5m]) > 0) * 100) or sum(up{job="cadvisor", node_id="${nodeId}"}) * 0`,
