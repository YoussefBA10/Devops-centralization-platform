...
export const QUERIES = {
  ...
  LOAD_AVERAGE_RATIO: (node: string) => 
    `(node_load1{instance=~"${node}(:.*)?"} / count(node_cpu_seconds_total{instance=~"${node}(:.*)?", mode="idle"})) or (up{job="node-exporter"} * 0)`,

  // H. CONTAINER UPTIME
  CONTAINER_UPTIME: (appId: string, appName: string) => 
    `(time() - container_start_time_seconds{container_label_com_monetique_app_id="${appId}"}) or (time() - container_start_time_seconds{name=~"/?${appName}"})`,

  // I. CPU USAGE TREND (Stacked Area)
  CPU_USAGE_STACKED: (appId: string, appName: string) => 
    `(rate(container_cpu_usage_seconds_total{container_label_com_monetique_app_id="${appId}"}[5m]) * 100 or rate(container_cpu_usage_seconds_total{name=~"/?${appName}"}[5m]) * 100) or (up{job="cadvisor"} * 0)`,

  // J. NETWORK THROUGHPUT
  NETWORK_THROUGHPUT: (appId: string, appName: string) => ({
    rx: `(rate(container_network_receive_bytes_total{container_label_com_monetique_app_id="${appId}"}[5m]) or rate(container_network_receive_bytes_total{name=~"/?${appName}"}[5m])) or (up{job="cadvisor"} * 0)`,
    tx: `(rate(container_network_transmit_bytes_total{container_label_com_monetique_app_id="${appId}"}[5m]) or rate(container_network_transmit_bytes_total{name=~"/?${appName}"}[5m])) or (up{job="cadvisor"} * 0)`
  }),
