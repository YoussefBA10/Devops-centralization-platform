...
export const MemoryAnalysis: React.FC<MemoryAnalysisProps> = ({ selectedNode, timeRange, triggerRefresh }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch Swap Total
        const swapTotalRes = await prometheus.queryInstantByKey('SWAP_TOTAL', { node: selectedNode });
        if (swapTotalRes && swapTotalRes.length > 0) {
          setSwapTotal(parseFloat(swapTotalRes[0].value[1]) || 0);
        }

        // Run range queries — only breakdown, %, and swap
        const [usedRaw, cachedRaw, buffersRaw, freeRaw, pctRes, swapUsageRes] = await Promise.all([
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      {/* 3. Swap Usage */}
      <Card className="bg-[#1a1d27] border-white/5 shadow-2xl lg:col-span-2">
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
